(ns app.modules.recipe-selector.components
  (:require ["@material-ui/core/Box" :default Box]
            ...
            [app.modules.recipe-selector.logic :as logic]
          ...
          [:> TableRow
           (for [c (range (count row))
             ...
             [:> TableCell (if (coll? cell)
                             (str (-> cell first c.item/id->name) " x " (second cell))
                             cell)])])]]]]))
...
(defn recipe-selector [{:keys [recipes]}]
  (let [recipes (logic/adapt-recipes recipes)
