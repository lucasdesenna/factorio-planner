(ns app.modules.factory-previewer.cards
  (:require [devcards.core :as dc :refer [defcard deftest]]
            [cljs.test :include-macros true :refer [is are testing]]
            [app.modules.factory-previewer.components :refer [factory-previewer]]
            [app.modules.factory-previewer.logic :as fac-pre.l]))
...
(defcard factory-previewer-card
  (dc/reagent #(factory-previewer {:factory-input [:item.iron-plate]
...
(deftest item-id->item
  (is (thrown? js/Error (fac-pre.l/item-id->item :non-existing-recipe-id all-items)))
  (are [id item] (= (fac-pre.l/item-id->item id all-items) item)
    :item.a item-a
    :item.b item-b
    :item.c item-c))
...
(deftest recipe-id->recipe
  (is (thrown? js/Error (fac-pre.l/recipe-id->recipe :non-existing-recipe-id all-recipes)))
  (are [id recipe] (= (fac-pre.l/recipe-id->recipe id all-recipes) recipe)
    :recipe.a recipe-a
    :recipe.b recipe-b
    :recipe.c recipe-c))
...
(deftest denormalize
  (testing "Denormalizes a given item-id"
    (are [item-id denormalized] (= (fac-pre.l/denormalize item-id {:ignored-ids #{}
                                                                   :all-items all-items
  ...
  (testing "Denormalizes a given recipe-id"
    (are [recipe-id denormalized] (= (fac-pre.l/denormalize recipe-id {:ignored-ids #{}
                                                                       :all-items all-items
                                                                       :all-recipes all-recipes}) denormalized)
  ...
  (testing "Ignores items and recipes which ids are contained in the ignored-id set"
    (are [id ignored-ids denormalized] (= (fac-pre.l/denormalize id {:ignored-ids ignored-ids
                                                                     :all-items all-items
                                                                     :all-recipes all-recipes}) denormalized)
      :item.a #{} denormalized-item-a
      :recipe.a #{} denormalized-recipe-a
...
(deftest denormalized-item->requirements
  (are [item requirements] (= (fac-pre.l/denormalized-item->requirements item) requirements)
    denormalized-item-a {:recipe.a {:assemblers 0.5
                                    :consumption-per-sec {:item.raw-a 0.5}}}
    denormalized-item-b {:recipe.a {:assemblers 4
                                    :consumption-per-sec {:item.raw-a 4}}
                         :recipe.b {:assemblers 2
                                    :consumption-per-sec {:item.a 8}}}
    denormalized-item-c {:recipe.a {:assemblers 4.5
